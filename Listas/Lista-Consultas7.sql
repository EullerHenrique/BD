...
CREATE OR REPLACE FUNCTION update_valor_emprestimo1(	
...
DECLARE
	...
	SELECT VALOR_EMPRESTIMO, JUROS_EMPRESTIMO FROM EMPRESTIMO 
...
select nome_cliente, count(*) from emprestimo 
group by nome_cliente order by count(*) desc
...
CREATE OR REPLACE FUNCTION update_valor_emprestimo2(	
	p_nome_cliente character varying
)
  RETURNS void as
$BODY$
DECLARE
    ...
    cursor_relatorio CURSOR FOR 
	SELECT VALOR_EMPRESTIMO, JUROS_EMPRESTIMO, NUMERO_EMPRESTIMO
    FROM EMPRESTIMO 
	WHERE NOME_CLIENTE=p_nome_cliente;
BEGIN
